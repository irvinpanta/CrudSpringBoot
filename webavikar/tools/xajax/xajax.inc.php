...
class xajax
{
	...
	function processRequests()
	{
		...
		if ($bFoundFunction)
		{
			...
			if (!$bEndRequest) {
				...
				# verificar que sys_usuario no llegue vacio. #29-01-2018 DMP
				if (!isset($_SESSION["sys_usuario"]) || $_SESSION["sys_usuario"] == "" || !isset($_SESSION["sys_session"])){
					if (function_exists("_interfazDesSeguridadAjaxLoadSession")){
						$verifica = false;
						if ($sFunctionName == "_interfazDesSeguridadAjaxLoadSession"){
							$verifica = true;
						}else{
							$oResponse = new xajaxResponse();
							$oResponse->addScript("
				...
				# Verificar que el usuario tenga acceso a la funcion.
				if ($verifica && function_exists("_verificarXAJAX")){
					$mReturn = call_user_func_array("_verificarXAJAX", array($sFunctionName));
					if (!$mReturn) {
						$verifica = false;
						$oResponse = new xajaxResponse();
						$oResponse->addAlert("Ud. No tiene el permiso para acceder a esta operación");
					}
				}
				if ($verifica) {
		...
		## Boton para ocultar/mostrar Mensajes flotantes: DMP - 11-04-2018 ###################
        $oResponse->addScript('indexMensajesDMP();');
        ######################################################################################
	...
	function getJavascriptInclude($sJsURI="", $sJsFile=NULL)
	{
		...
		{
			$html .= "\t<script type=\"text/javascript\">\n";
			$html .= "window.setTimeout(function () { if (!xajaxLoaded) { alert('Error: xajax no ha encontrado el archivo javascript. Verifique si esta ruta es correcta: {$sJsURI}{$sJsFile}'); } }, ".$this->iTimeout.");\n";
			$html .= "\t</script>\n";
		}
		return $html;
	}
